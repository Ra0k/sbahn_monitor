...
df = pd.DataFrame({
    "Station": df['station_name'],"Verspätung": df['delay'],"Abfahrtszeit": df["departure_time"],"Endstation":df["destination"]
})
...
app.layout = html.Div(children=[
    ...
    html.Div(children=str(str(int(round(avgdelaynumber*100)))+'% of S-Bahns delayed in total on this line. '), style={
        #'textAlign': 'center',
        #'color': colors['text']
...
def update_figure(value):
    ...
    if value == 'Flughafen München':
        ...
        order = ['Herrsching', 'Seefeld-Hechendorf', 'Steinebach', 'Weßling', 'Neugilching','Gilching-Argelsried','Geisenbrunn',
                 'Germering-Unterpfaffenhofen','Harthaus','Freiham', 'Neuaubing',
                 'Westkreuz','Pasing','Laim','Hirschgarten','Donnersbergerbrücke','Hackerbrücke','Hauptbahnhof','Karlsplatz',
                 'Marienplatz','Isartor','Rosenheimer Platz','Ostbahnhof München','Leuchtenbergring', 'Daglfing','Englschalking','Johanneskirchen','Unterföhring','Ismaning', 'Hallbergmoos','Flughafen Besucherpark']
